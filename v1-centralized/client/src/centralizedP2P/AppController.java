...
import centralizedP2P.RunnableThread.Client;
...
public class AppController {
    ...
    public static boolean connectToServer(String url) {
        try {
            ...
            boolean available = WebServer.connectToServer();
            if (!available) {
                return false;
            }
            ...
            java.net.ServerSocket serverSocket = new java.net.ServerSocket(myAddress.getPort(), 0, myAddress.getAddress());
            Thread t = new Thread(new Listener(serverSocket));
            t.start();
    ...
    public void initialize() {
        try {
            addressLabel.setText("My address: " + myAddress.getAddress().getHostAddress() + ":" + myAddress.getPort());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
            ...
            // login to existing account
                ...
                switch (response) {
                    case 200:
                        ...
                        // Load all books that I'd shared previously
                    ...
                    default:
                        // username already logged in
                        lsTextAlert.setText("Something is wrong! Please try again later!");
                        break;
                }
            }
            // Log out
        ...
        public SharedBookCell() {
            ...
            updateLocButton.setOnAction(e -> {
                ...
                if (selectedFile != null) {
                    ...
                    if (status == true) {
//                        try {
//                            Listener serverSocket = new Listener(currentBook.getPort());
//                            Thread t = new Thread(new Server(serverSocket, selectedFile.toString()));
//                            t.start();
//                        } catch (Exception ex) {
//                            ex.printStackTrace();
//                        }
        ...
        protected void updateItem(Book newBook, boolean empty) {
            ...
            if (empty) {
                ...
                if (!file.exists()) { // if file doesn't exist, show option to update location
                    ...
                    // Unshare this book
                    WebServer.updateBookSharingStatus(currentBook, false);
                } else {
